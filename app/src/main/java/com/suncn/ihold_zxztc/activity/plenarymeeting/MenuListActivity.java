...
import com.suncn.ihold_zxztc.adapter.MenuListExAdapter;
...
public class MenuListActivity extends BaseActivity {
    ...
    private ArrayList<MenuListExAdapter> adapters;
    ...
    public void initData() {
        ...
        if (bundle != null) {
            strHotelId = bundle.getString("strHotelId", "");
        }
        setHeadTitle("菜单");
        getDateList();
    }
    ...
    private void getListData() {
        prgDialog.showLoadDialog();
        textParamMap = new HashMap<>();
        textParamMap.put("strHotelId", strHotelId);
        textParamMap.put("dtEatTime", strDate);
        doRequestNormal(ApiManager.getInstance().MenuManagerServlet(textParamMap), 1);
    }
...
//        try {
        switch (what) {
            case 0:
                ...
                break;
            case 1:
                MenuListBean menuListBean = (MenuListBean) object;
                ...
                MenuListExAdapter adapter = adapters.get(currentIndex);
                if (adapter == null) {
                    adapter = new MenuListExAdapter(activity, menuListBean.getObjList());
                    expandableListView.setAdapter(adapter);
                } else {
                    adapter.setDutyLists(menuListBean.getObjList());
                    adapter.notifyDataSetChanged();
                }
                if (menuListBean.getObjList().size() == 0 || menuListBean.getObjList() == null) {
                    empty_TextView.setVisibility(View.VISIBLE);
                } else {
                    empty_TextView.setVisibility(View.GONE);
                }
                if (menuListBean.getObjList().size() > 0) {
...
//        }
    ...
    private void initAddView() {
        View view = LayoutInflater.from(activity).inflate(R.layout.layout_expanlistview, null);
        MenuListExAdapter adapter = new MenuListExAdapter(activity, new ArrayList<>());
        MyExListViewForScrollView recyclerView = view.findViewById(R.id.myExListView);
